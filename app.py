...
def send_message(
    ...
    if not handoff.used_ai:
        raise DispatchModelError(handoff.note or "调度大模型未返回有效任务分析")
            ...
            if action == "logout":
                ...
                try:
                    ...
                    st.session_state["city_dispatch_error"] = (
                        f"调度大模型调用失败：{str(exc)[:220]}。操作票未下发。"
                    )
                except Exception as exc:
                    st.session_state["city_dispatch_error"] = (
...
def render_county_dashboard(county: str, city_name: str, username: str) -> None:
    ...
    html = dedent(
        ...
        function playSavedVoice(i){const voice=visibleMessages[i].voiceConfirmation;if(!voice?.audio_data_url)return;if(savedVoiceAudio){savedVoiceAudio.pause();savedVoiceAudio.currentTime=0;savedVoiceAudio=null;const wasSame=savedVoiceIndex===i;savedVoiceIndex=-1;render();if(wasSame)return}savedVoiceIndex=i;savedVoiceAudio=new Audio(voice.audio_data_url);savedVoiceAudio.onended=savedVoiceAudio.onerror=()=>{savedVoiceAudio=null;savedVoiceIndex=-1;render()};savedVoiceAudio.play();render()}
        async function refreshMicrophones(){
          if(!navigator.mediaDevices?.enumerateDevices)return;
          const select=document.getElementById("microphoneSelect"),current=select.value;
          const devices=(await navigator.mediaDevices.enumerateDevices()).filter(d=>d.kind==="audioinput");
          select.innerHTML='<option value="">系统默认麦克风</option>'+devices.map((d,i)=>`<option value="${esc(d.deviceId)}">${esc(d.label||`麦克风 ${i+1}`)}</option>`).join("");
        ...
        function openVoiceModal(id){
          ...
          document.getElementById("voiceBack").classList.add("show");renderAttempts();refreshMicrophones()
        }
        function closeVoiceModal(){if(mediaRecorder?.state==="recording")mediaRecorder.stop();stopAllPlayback();document.getElementById("voiceBack").classList.remove("show")}
    ...
    *{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei UI","PingFang SC",sans-serif;overflow:hidden}
    ...
    function showGlobalProcessing(title,detail,timeoutMs=185000){
      ...
      globalProcessingTimer=setTimeout(()=>{
        ...
        document.getElementById("globalProcessingDetail").innerHTML="后端未在限定时间内返回。可能是调度大模型、网络连接或共享任务库调用异常。<br>本次前端等待已停止，请关闭提示后查看错误信息或重新测试。";
      },timeoutMs);
    }
    function raiseFonts(){}
...
if isinstance(network_selection, dict):
    ...
    if selection_nonce != st.session_state.get("processed_network_selection"):
        ...
        if network_selection.get("action") == "testModel":
            model_ok, model_note = ARK_AGENT.test_connection(timeout_seconds=180)
            if model_ok:
