...
html = dedent(
    ...
    *{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei UI","PingFang SC",sans-serif;overflow:hidden}
    ...
    #route.route{margin:12px;padding:14px 13px;border-color:#d8e7e3;background:#fbfdfc}.route .rnode{min-height:42px;gap:11px}.route .rnode .mini{width:32px;height:32px;border:1px solid #8fc9ba;background:#f4faf8!important;color:#267061;font-size:9px;box-shadow:none}.route .rnode.current .mini{border-color:#65b69f;background:#e5f4ef!important;color:#00745e}.route .rnode small{margin-bottom:2px;color:#78918b;font-size:9px}.route .rnode b{color:#25483f;font-size:12px;font-weight:600}.route .flow{height:27px;margin-left:16px;border-left:1px dashed #a8cfc5}.route .flow:after{width:5px;height:5px;left:-3px;background:#69bda7;box-shadow:none;animation-duration:1.8s}.route .flow em{left:13px;top:8px;color:#7e9690;font-size:8px}
    ...
    function render(){
      ...
      const c=cities[active];document.getElementById("route").innerHTML=`<div class="rnode source"><span class="mini">省</span><div><small>指令发起</small><b>黑龙江省调度智能体</b></div></div><div class="flow"><em>专线传输</em></div><div class="rnode current"><span class="mini">${c.short}</span><div><small>当前接收</small><b>${c.name}调度智能体</b></div></div><div class="flow"><em>辖区独立链路</em></div><div class="rnode target"><span class="mini">区</span><div><small>目标节点</small><b>${selected}智能体</b></div></div>`;
    ...
    function selectCity(i){const key=`city:${i}`,secondClick=deliberateSecondClick(key);active=i;selected=cities[i].counties[0];armedKey=key;focused=secondClick;saveNetworkSelection();render()}
