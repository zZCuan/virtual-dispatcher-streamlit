...
第三项，复核并向哈尔滨市调回令。</textarea></div><button class="send" onclick="sendDownstream()">确认并下发操作票</button></section></div>
        ...
        setInterval(()=>{if(speaking<0&&!document.getElementById("modal").classList.contains("show")&&!document.getElementById("processing").classList.contains("show"))post({action:"refresh"})},5000);
        </script></body></html>
        """
