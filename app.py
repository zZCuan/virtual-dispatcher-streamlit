...
html = dedent(
    ...
    *{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei UI","PingFang SC",sans-serif;overflow:hidden}
    ...
    .scene.focused .knode.focusCounty{width:54px;height:32px;z-index:6}.scene.focused .knode.focusCounty>span{width:11px;height:11px;background:#fff;border:2px solid #00a779;box-shadow:0 2px 8px rgba(0,127,102,.18)}.scene.focused .knode.focusCounty small{left:29px;top:5px;padding:3px 7px;border:1px solid #d3e7e2;border-radius:9px;background:rgba(255,255,255,.96);color:#285f52;font-size:10px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(20,80,66,.08)}.scene.focused .knode.focusCounty.labelLeft small{left:auto;right:29px}.scene.focused .knode.focusCounty.labelTop small{left:50%;right:auto;top:auto;bottom:30px;transform:translateX(-50%);text-align:center}.scene.focused .knode.focusCounty.labelBottom small{left:50%;right:auto;top:30px;transform:translateX(-50%);text-align:center}.scene.focused .knode.focusCounty.labelTop.labelFar small{bottom:48px}.scene.focused .knode.focusCounty.labelBottom.labelFar small{top:48px}.scene.focused .knode.focusCounty.selected>span{background:#f4b63d;border-color:#fff}
    ...
    function render(){
      ...
      cities.forEach((c,ci)=>c.counties.forEach((name,countyIndex)=>{
        ...
        const staggerClass=verticalLabel&&countyIndex%2===1?"labelFar":"";
    ...
    function selectCounty(ci,name,event){
      ...
      const key=`county:${ci}:${name}`,secondClick=deliberateSecondClick(key);active=ci;selected=name;armedKey=key;focused=secondClick;saveNetworkSelection();render()
    }
