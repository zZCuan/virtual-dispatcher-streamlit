...
def render_city_dashboard(city_name: str, counties: list[str], username: str) -> None:
    ...
    html = dedent(
        ...
        *{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei UI","PingFang SC",sans-serif;overflow:hidden}
        ...
        <header class="top"><div class="brand"><b>龙江电网 · 哈尔滨市虚拟配网调度中心</b><small>HARBIN VIRTUAL DISPATCH NETWORK · 当前账号：harbin_city（市级调度权限）</small></div><div style="display:flex;gap:8px"><button class="logout" onclick="post({action:'refresh'})">刷新数据</button><button class="logout" onclick="post({action:'logout'})">退出账号</button></div></header>
...
第三项，复核并向哈尔滨市调回令。</textarea></div><button class="send" onclick="sendDownstream()">确认并下发操作票</button></section></div>
        <div class="processing" id="processing"><section class="processingCard"><div class="processingSpinner"></div><b>市级智能体正在处理操作票</b><p>正在校验管辖范围、细化区县任务并写入共享任务库<br>请稍候，不要重复点击或关闭页面</p></section></div>
        <script>
        ...
        function sendDownstream(){const button=document.querySelector(".modal .send");if(button.disabled)return;button.disabled=true;button.textContent="正在下发…";const payload={action:"downstream",county:document.getElementById("targetCounty").value,title:document.getElementById("taskTitle").value,steps:document.getElementById("taskSteps").value};closeModal();document.getElementById("processing").classList.add("show");post(payload)}
...
html = dedent(
    ...
    *{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei UI","PingFang SC",sans-serif;overflow:hidden}
    ...
    .scene.focused .knode.focusCounty{width:54px;height:32px;z-index:6}.scene.focused .knode.focusCounty>span{width:11px;height:11px;background:#fff;border:2px solid #00a779;box-shadow:0 2px 8px rgba(0,127,102,.18)}.scene.focused .knode.focusCounty small{left:29px;top:5px;padding:3px 7px;border:1px solid #d3e7e2;border-radius:9px;background:rgba(255,255,255,.96);color:#285f52;font-size:10px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(20,80,66,.08)}.scene.focused .knode.focusCounty.labelLeft small{left:auto;right:29px}.scene.focused .knode.focusCounty.selected>span{background:#f4b63d;border-color:#fff}
